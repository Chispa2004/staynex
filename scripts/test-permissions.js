...
const { getUserDisplayName, getHotelGreeting } = await import('../dashboard/lib/user-presentation.js');
assert.equal(getUserDisplayName(null), null);
assert.equal(getUserDisplayName({ user_metadata: { full_name: 'Unverified' } }), null, 'An absent session identity must not display a profile');
const sessionUser = { id: 'synthetic-user', email: 'chema@example.invalid', user_metadata: { full_name: 'Chema García', display_name: 'Chema', first_name: 'José' } };
assert.equal(getUserDisplayName(sessionUser), 'Chema García');
assert.equal(getUserDisplayName({ ...sessionUser, user_metadata: { full_name: ' ', display_name: 'Chema' } }), 'Chema');
assert.equal(getUserDisplayName({ ...sessionUser, user_metadata: { first_name: 'José' } }), 'José');
assert.equal(getUserDisplayName({ ...sessionUser, user_metadata: { full_name: 7 } }), 'chema');
assert.equal(getUserDisplayName({ id: 'synthetic-user' }), null);
assert.equal(getHotelGreeting('Europe/Madrid', new Date('2026-09-07T08:00:00Z')), 'Buenos días');
assert.equal(getHotelGreeting('Europe/Madrid', new Date('2026-09-07T14:00:00Z')), 'Buenas tardes');
assert.equal(getHotelGreeting('Europe/Madrid', new Date('2026-09-07T22:00:00Z')), 'Buenas noches');
assert.equal(getHotelGreeting('America/New_York', new Date('2026-09-07T14:00:00Z')), 'Buenos días', 'Greeting uses the hotel timezone');
assert.equal(getHotelGreeting('invalid/timezone'), 'Bienvenido');
assert.match(appShellSource, /const isPrimaryNavigation = group.id === 'operations';[\s\S]*?const isOpen = isPrimaryNavigation \|\| openGroups\[group.id\]/, 'Primary navigation must remain expanded regardless of saved accordion state');
assert.match(appShellSource, /!isPrimaryNavigation \? <button/, 'Primary navigation must not expose a collapse control');
const sessionDisplaySource = readFileSync(join(root, 'dashboard/lib/use-session-display-name.js'), 'utf8');
assert.match(sessionDisplaySource, /onAuthStateChange/, 'Displayed identity follows logout and session changes');
assert.match(sessionDisplaySource, /active && !authChanged/, 'Late initial session reads cannot overwrite a newer identity');
assert.doesNotMatch(sessionDisplaySource, /setRole|setPermissions|localStorage|fetch\(/, 'Presentation must not change authorization or store identity');
const languageSelectorSource = readFileSync(join(root, 'dashboard/components/LanguageSelector.js'), 'utf8');
assert.match(languageSelectorSource, /aria-expanded=\{visible\}/);
assert.match(languageSelectorSource, /event.key === 'Escape'/);
assert.match(languageSelectorSource, /DASHBOARD_LANGUAGES.filter\(\(item\) => item.code !== language\)/);
console.log('Receptionist permissions and dashboard presentation checks passed');
