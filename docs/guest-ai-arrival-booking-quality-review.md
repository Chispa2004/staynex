...
- Primera ronda: mismas 36 entradas antes/después (72), más cuatro comparaciones Concierge (8). Base 5237113 frente a 2a572d13. Segunda ronda: 12 generaciones focalizadas sobre las instrucciones de fcf2603. Modelo efectivo `gpt-4.1-mini-2025-04-14`, sin errores del proveedor. La presentación final reproducida corresponde a 5b044c2; los ajustes posteriores a fcf2603 solo afectan presentación y pruebas, no esas solicitudes al proveedor.
...
- 16 grupos nuevos `test:arrival-booking-quality`: recuperación compuesta real, fechas/zona horaria, aislamiento, información restringida, URLs, ofertas, capacidades, borradores, seguimiento, garantías de habitación, adaptadores principal/Concierge con SDK simulado, decisiones humanas, datos faltantes, prioridad de emergencia y ausencia de falsa clasificación de mantenimiento en una consulta de nueva estancia.
...
Primera publicación: [PR22](https://github.com/Chispa2004/staynex/pull/22), SHA integrado `668d0b33b33458dcd74fd355fedce3de64f8d969`. [CI del SHA final de PR](https://github.com/Chispa2004/staynex/actions/runs/36119850627) y [CI de main](https://github.com/Chispa2004/staynex/actions/runs/36120121372): los tres jobs PASS; logs confirman 15 grupos nuevos y 15 previos. Revisión propia del diff y ausencia de revisiones bloqueantes; no se atribuye aprobación a un revisor externo inexistente.

Vercel Production `97ywqHFwXtf4BibxLuSbqfUdKDbF` Ready y asociado al dominio público; Railway `07460572-3149-45a5-9bb5-0218e2aef7f3` Active, una réplica, despliegue anterior Removed. El proceso activo acredita el SHA integrado, `SEND_AUTOMATIONS=false`, Guest Memory ausente con valor efectivo false por contrato, `USE_MOCK_AI=false` y Concierge true sin cambios. Supabase permanece en el proyecto previsto. `/health` devuelve HTTP 200 healthy; esto solo acredita salud del backend, no generación ni entrega.

En Inbox autenticado se comprobaron las 15 conversaciones SIMULADO y el borrador nuevo de llegada, sin pulsar Copiar/Enviar ni cambiar control humano. Los 42 mensajes y las once tablas del inventario privado limitado al hotel demo son idénticos antes/después; siguen intactas las seis respuestas de Nora, Inés y Dani. El Dashboard público muestra Guest Memory desactivado.

La revisión pública detectó una interpretación innecesaria de «hasta la mañana» como el día siguiente. Se añadió una regresión y un ajuste mínimo que distingue esa expresión de «mañana a las 00:30», manteniendo la pregunta sobre la fecha de llegada. En la consulta de próxima visita, la recomendación auxiliar indicaba mantenimiento porque buscaba `ac` dentro de `gracias`; se limita ese acrónimo a una palabra completa y se verifica que una consulta real de AC continúa reconocida. Estos cambios corresponden a [PR23](https://github.com/Chispa2004/staynex/pull/23); su publicación y repetición visual se documentarán al terminar. No confundir despliegue con evaluación funcional del modelo ni borrador leído con mensaje enviado.
