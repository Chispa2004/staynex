...
## Revisión de publicación — 23/09/2026

PR: https://github.com/Chispa2004/staynex/pull/16. Head funcional/documental revisado: `6717d4da94ce60c678bd3a16d0d29be3ed029c6e`. CI de PR https://github.com/Chispa2004/staynex/actions/runs/35852408207 y de push https://github.com/Chispa2004/staynex/actions/runs/35852401902: tres jobs SUCCESS, ninguno omitido/cancelado. Logs comprobados: Critical ejecuta realmente los 8 grupos de comportamiento; PostgreSQL ejecuta los 8 grupos SQL; Dashboard compila. Sin revisiones ni comentarios bloqueantes en la consulta realizada. Preview ACqkmbA4iuNW38NcVcHUya67pXyS Ready para ese SHA; no constituye prueba funcional.

Preflight READ ONLY ejecutado en Supabase `vblxmnqbatqrynfaasmf`, rol postgres: columnas/tipos/nullabilidad/defaults compatibles, unicidad de hotel_id para estado y de slugs, user_id nullable para invitaciones. No existen tabla de operaciones ni RPC nuevas; no hay triggers de usuario en las tres tablas. RLS está habilitado, sin políticas en ellas: no se modifican sus permisos. Definiciones, ACL, restricciones, índices y huellas de conservación (10 hoteles, 12 asignaciones, 10 estados) guardados privadamente, fuera de Git. La reproducción del catálogo leído en PostgreSQL desechable también PASS con la migración exacta, altas workspace/Platform, repetición, finalización y flags OFF. Ninguna escritura de esta prueba fue remota.

Railway Production tiene una réplica visible; despliegue `fa8fb1ed-5f40-4880-a623-b861115af708`, proceso src/server.js con SHA `b755ed61cd60211bcf65a1c63d018b4514673aa4`, proyecto efectivo `vblxmnqbatqrynfaasmf`, SEND_AUTOMATIONS=false y GUEST_MEMORY_ENABLED ausente (OFF por contrato). Vercel no contiene GUEST_MEMORY_ENABLED en Project/Shared. SUPABASE_URL figura para Production y Preview, pero es secreto de solo escritura: no puede acreditarse su valor desde esa pantalla. Se canceló la edición sin guardar.

**Puerta pendiente antes de SQL/merge:** lectura de projectRef del endpoint de diagnóstico de servidor ya existente en Preview; su redirección SSO es bloqueada por el navegador automatizado. Se pidió al usuario únicamente abrir ese acceso y comunicar la referencia pública, sin secretos ni cambios de protección. No se sustituye esta comprobación por variables locales. La sesión pública de aplicación disponible es Recepción y no se elevará ni suplantará para ensayar el alta.

Hasta resolver esa puerta: migración NO aplicada, PR abierta sin merge/auto-merge, producción continúa en b755ed6, ningún hotel técnico creado. Los puntos 5 y 2 no se marcan verificados en producción. La verificación administrativa pendiente requiere una sesión auténtica con permisos de alta y operación sobre el único hotel técnico autorizado. No se han cambiado flags, configuración, proveedores, demo ni datos existentes.
