...
const defaultForm = (provider, connection) => ({
  ...
  notes: connection?.metadata?.notes || '',
...
export const PmsConnectionsClient = () => {
  ...
  const save = async (event) => {
    ...
    try {
      ...
      setFeedback({
        type: 'success',
        message: body.connection?.sync_status === 'pending_setup'
          ? 'Configuration saved. Staynex team must activate the live connector before syncing.'
          : 'PMS connection saved.'
      });
  ...
  const testConnection = async (provider) => {
    ...
    try {
      ...
      setFeedback({
        type: 'success',
        message: body.status === 'pending_setup'
          ? body.message || 'Configuration saved. Staynex team must activate the live connector before syncing.'
          : `${provider.name} connection works.`
      });
      await loadConnections();
    } catch (error) {
      setFeedback({ type: 'error', message: error.message });
  ...
  const syncReservations = async (provider) => {
    ...
    try {
      ...
      setFeedback({
        type: 'success',
        message: body.summary?.status === 'pending_setup'
          ? body.summary.message
          : `Sync complete: fetched ${body.summary?.totalFetched || body.summary?.fetched || 0}, processed ${body.summary?.totalProcessed || body.summary?.synced || 0}, skipped ${body.summary?.totalSkipped || body.summary?.skipped || 0}.`
      });
      await loadConnections();
    } catch (error) {
      setFeedback({
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {(providers.length ? providers : [{ key: 'apaleo', name: 'Apaleo', status: 'live_api', statusLabel: 'Live API', configurationMode: 'live_api', defaultBaseUrl: 'https://api.apaleo.com' }]).map((provider) => (
          <PmsProviderCard
            key={provider.key}
            provider={provider}
