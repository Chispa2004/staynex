...
export const PmsConnectionForm = ({
  ...
  const requestActivation = (event) => {
    ...
    window.setTimeout(() => event.currentTarget.form?.requestSubmit(), 0);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 backdrop-blur-sm">
      <form onSubmit={onSave} className={isLight ? 'w-full max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl shadow-slate-300/40' : 'w-full max-w-2xl rounded-2xl border border-white/10 bg-[#0b1019] p-6 shadow-2xl shadow-black/40'}>
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className={isLight ? 'text-lg font-semibold text-slate-950' : 'text-lg font-semibold text-white'}>{initialConnection ? 'Manage' : 'Connect'} {provider.name}</p>
            <p className={isLight ? 'mt-1 text-sm text-slate-500' : 'mt-1 text-sm text-slate-500'}>
              {liveApi ? 'Secrets are encrypted before they are stored.' : 'Configuration can be saved now; the Staynex team must activate the live connector before syncing.'}
            </p>
          </div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <label className="space-y-2">
            <span className={labelClass}>Connection mode</span>
            <select
              className={inputClass}
              value={form.connection_mode}
              onChange={(event) => setForm((current) => ({ ...current, connection_mode: event.target.value }))}
            >
              <option value="live_api">Live API</option>
              <option value="manual_setup">Manual setup</option>
              <option value="csv_import">CSV/import mode</option>
              <option value="activation_request">Request activation</option>
            </select>
          </label>
          <label className="space-y-2">
          ...
          <label className="space-y-2 sm:col-span-2">
            ...
            <textarea
              ...
              placeholder="Integration notes, contact person, CSV/import details or connector activation request."
            />
          </label>
